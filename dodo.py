...
from zs.utils.fmt import fmt
from zs.utils.shell import cd, call, rglob, globs, which
...
def pyfiles():
    with cd(REPOROOT):
        return sorted(glob('zs/**/*.py', recursive=True))
...
def task_pycov():
    ...
    return dict(
        actions=[
            fmt('pytest {PYTHON} -m pytest -s -vv --cov={ZSDIR} {TESTDIR}'),
        ],
    )
